...
class RDNet_Base_SAttention(nn.Module):
    ...
    def __init__(self, num_classes: int, sa_kernel_size: int = 7):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # self.model.head 包含 global_pool 和 norm
        # 注意：在 timm 中，如果 num_classes=0，self.model.head(x) 
        # 通常會返回經過 global pooling 但尚未經過 fc 的特徵。
        # 由於您在 __init__ 中定義了 self.global_pool，但未使用它，
        # 我們假設 self.model.head(x) 包含了 pooling 步驟。
    ...
    def freeze_and_unfreeze_params(self, unfreeze_start_module=None):
        ...
        print(f"✅ 凍結完成: SA模組和FC層已解凍。第一個 ESE/SA 之前的參數被凍結。")
